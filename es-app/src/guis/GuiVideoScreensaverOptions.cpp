...
GuiVideoScreensaverOptions::GuiVideoScreensaverOptions(Window* window, const char* title) : GuiScreensaverOptions(window, title)
{
	...
	auto ss_info = std::make_shared< OptionListComponent<std::string> >(mWindow, u8"显示游戏信息", false);
	...
	addWithLabel(u8"屏保显示游戏信息", ss_info);
	...
	addWithLabel(u8"兼容低分辨率模式", captions_compatibility);
	...
	addWithLabel(u8"屏保视频自动缩放", stretch_screensaver);

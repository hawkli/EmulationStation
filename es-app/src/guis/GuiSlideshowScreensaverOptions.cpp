...
GuiSlideshowScreensaverOptions::GuiSlideshowScreensaverOptions(Window* window, const char* title) : GuiScreensaverOptions(window, title)
{
	...
	addWithLabel(row, u8"(秒)后切换图片", sss_image_sec);
	...
	addWithLabel(row, u8"全屏缩放图片", sss_stretch);
	...
	addEditableTextComponent(row, u8"背景音效", sss_bg_audio_file, Settings::getInstance()->getString("SlideshowScreenSaverBackgroundAudioFile"));
	...
	addWithLabel(row, u8"自定义图片", sss_custom_source);
	...
	addEditableTextComponent(row, u8"自定义图片目录", sss_image_dir, Settings::getInstance()->getString("SlideshowScreenSaverImageDir"));
	...
	addWithLabel(row, u8"图片目录层级开关.", sss_recurse);
	...
	addEditableTextComponent(row, u8"图片格式筛选", sss_image_filter, Settings::getInstance()->getString("SlideshowScreenSaverImageFilter"));

...
GuiSlideshowScreensaverOptions::GuiSlideshowScreensaverOptions(Window* window, const char* title) : GuiScreensaverOptions(window, title)
{
	...
	addWithLabel(row, " (秒)后切换图片", sss_image_sec);
	...
	addWithLabel(row, "全屏缩放图片", sss_stretch);
	...
	addEditableTextComponent(row, "背景音效", sss_bg_audio_file, Settings::getInstance()->getString("SlideshowScreenSaverBackgroundAudioFile"));
	...
	addWithLabel(row, "自定义图片", sss_custom_source);
	...
	addEditableTextComponent(row, "自定义图片目录", sss_image_dir, Settings::getInstance()->getString("SlideshowScreenSaverImageDir"));
	...
	addWithLabel(row, "图片目录层级开关", sss_recurse);
	...
	addEditableTextComponent(row, "图片格式筛选", sss_image_filter, Settings::getInstance()->getString("SlideshowScreenSaverImageFilter"));

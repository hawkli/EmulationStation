	...
	mName(window)
{
	...
	mLblRating.setText("评　　分: ");
	addChild(&mLblRating);
	addChild(&mRating);
	mLblReleaseDate.setText("发布年份: ");
	addChild(&mLblReleaseDate);
	addChild(&mReleaseDate);
	mLblDeveloper.setText("开发厂商: ");
	addChild(&mLblDeveloper);
	addChild(&mDeveloper);
	mLblPublisher.setText("发行厂商: ");
...
void GridGameListView::addPlaceholder()
{
	...
	FileData* placeholder = new FileData(PLACEHOLDER, "<找不到条目>", this->mRoot->getSystem()->getSystemEnvData(), this->mRoot->getSystem());
...
std::vector<HelpPrompt> GridGameListView::getHelpPrompts()
{
	...
	if(Settings::getInstance()->getBool("QuickSystemSelect"))
		prompts.push_back(HelpPrompt("lr", "主机选择"));
	prompts.push_back(HelpPrompt("up/down/left/right", "变更"));
	prompts.push_back(HelpPrompt("a", "启动"));
	prompts.push_back(HelpPrompt("b", "返回"));
	if(!UIModeController::getInstance()->isUIModeKid())
		prompts.push_back(HelpPrompt("select", "选项"));

	...
	mSearchType(type)
{
	...
	mResultName = std::make_shared<TextComponent>(mWindow, u8"结果名称", Font::get(FONT_SIZE_MEDIUM), 0x777777FF);
	...
	mResultDesc = std::make_shared<TextComponent>(mWindow, u8"结果排序", Font::get(FONT_SIZE_SMALL), 0x777777FF);
	...
	mMD_Pairs.push_back(MetaDataPair(std::make_shared<TextComponent>(mWindow, u8"评　　分:", font, mdLblColor), mMD_Rating, false));
	mMD_Pairs.push_back(MetaDataPair(std::make_shared<TextComponent>(mWindow, u8"发布年份:", font, mdLblColor), mMD_ReleaseDate));
	mMD_Pairs.push_back(MetaDataPair(std::make_shared<TextComponent>(mWindow, u8"开发厂商:", font, mdLblColor), mMD_Developer));
	mMD_Pairs.push_back(MetaDataPair(std::make_shared<TextComponent>(mWindow, u8"发行厂商:", font, mdLblColor), mMD_Publisher));
	mMD_Pairs.push_back(MetaDataPair(std::make_shared<TextComponent>(mWindow, u8"类　　型:", font, mdLblColor), mMD_Genre));
	mMD_Pairs.push_back(MetaDataPair(std::make_shared<TextComponent>(mWindow, u8"玩家数量:", font, mdLblColor), mMD_Players));
...
void ScraperSearchComponent::onSearchDone(const std::vector<ScraperSearchResult>& results)
{
	...
	if(results.empty())
	{
		...
		{
			mWindow->pushGui(new GuiMsgBox(mWindow, Utils::String::toUpper(u8"抓取配置不可用.\n请重新配置抓取方案."),
				u8"完成", mSkipCallback));
		}
		else
		{
			ComponentListRow row;
			row.addElement(std::make_shared<TextComponent>(mWindow, u8"未搜寻到结果-跳过", font, color), true);
		...
		{
			row.elements.clear();
			row.addElement(std::make_shared<TextComponent>(mWindow, Utils::String::toUpper(results.at(i).mdl.get("名称 ")), font, color), true);
...
void ScraperSearchComponent::onSearchError(const std::string& error)
{
	...
	mWindow->pushGui(new GuiMsgBox(mWindow, Utils::String::toUpper(error),
		u8"重试", std::bind(&ScraperSearchComponent::search, this, mLastSearch),
		u8"跳过", mSkipCallback,
		u8"取消", mCancelCallback));
}
...
std::vector<HelpPrompt> ScraperSearchComponent::getHelpPrompts()
{
	...
	if(getSelectedIndex() != -1)
		prompts.push_back(HelpPrompt("a", u8"确认结果"));
